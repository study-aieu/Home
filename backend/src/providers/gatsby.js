/**
 * Gatsby Provider Adapter
 * 
 * Supports publishing to Gatsby static site generator
...
class GatsbyAdapter {
  constructor() {
    ...
    this.supports = {
      images: true,
      tags: true,
      scheduling: false, // Gatsby doesn't support scheduling
  ...
  /**
   * Verify connection to Gatsby project
   * @param {object} auth - Authentication credentials
   * @returns {Promise<{ok: boolean, details?: any}>}
   */
  async verifyConnection(auth) {
    try {
      const { projectPath, gitRemote } = auth;
      
      if (!projectPath) {
        return {
          ok: false,
          details: 'Missing required credentials: projectPath'
        };
      }

      // Check if Gatsby project exists
      try {
        ...
        if (!packageExists) {
          return {
            ok: false,
            details: 'Gatsby project not found at specified path'
          };
        }

        // Check if it's a valid Gatsby project
        ...
        if (!packageJson.dependencies?.gatsby && !packageJson.devDependencies?.gatsby) {
          return {
            ok: false,
            details: 'Invalid Gatsby project - missing gatsby dependency'
          };
        }

        // Check if posts directory exists
        const postsPath = path.join(projectPath, 'content', 'posts');
        ...
        const gatsbyVersion = packageJson.dependencies?.gatsby || packageJson.devDependencies?.gatsby;
        ...
        return {
          ok: false,
          details: `Failed to verify Gatsby project: ${error.message}`
        };
      }
    } catch (error) {
      return {
  ...
  /**
   * List posts from Gatsby project
   * @param {object} auth - Authentication credentials
  ...
  async listPosts(auth, opts = {}) {
    try {
      ...
      console.error('Failed to list Gatsby posts:', error);
      throw new Error(`Failed to fetch posts: ${error.message}`);
    }
  }

  /**
   * Create a new post on Gatsby project
   * @param {object} auth - Authentication credentials
    ...
    try {
      ...
      if (gitRemote) {
        try {
          ...
          await execAsync(`git commit -m "Add blog post: ${input.title}"`, { cwd: projectPath });
      ...
      return {
        ...
        message: 'Post created successfully on Gatsby project',
        metadata: {
          filename: filename,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to create Gatsby post:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Update an existing post on Gatsby project
   * @param {object} auth - Authentication credentials
   * @param {string} postId - Gatsby post filename
  ...
  async updatePost(auth, postId, input) {
    try {
      ...
      if (gitRemote) {
        try {
          await execAsync('git add .', { cwd: projectPath });
          await execAsync(`git commit -m "Update blog post: ${input.title || postId}"`, { cwd: projectPath });
      ...
      return {
        ...
        postId: postId,
        externalId: postId,
        url: `https://yourdomain.com/blog/${postId.replace(/\.(md|mdx)$/, '')}`,
        message: 'Post updated successfully on Gatsby project',
        metadata: {
          filename: postId,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to update Gatsby post:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Delete a post from Gatsby project
   * @param {object} auth - Authentication credentials
   * @param {string} postId - Gatsby post filename
  ...
  async deletePost(auth, postId) {
    try {
      ...
      if (gitRemote) {
        try {
          await execAsync('git add .', { cwd: projectPath });
          await execAsync(`git commit -m "Delete blog post: ${postId}"`, { cwd: projectPath });
      ...
      return {
        ok: true,
        message: 'Post deleted successfully from Gatsby project',
        metadata: {
          gitCommit: gitCommit
        }
      };
    } catch (error) {
      console.error('Failed to delete Gatsby post:', error);
      return {
        ok: false,
  ...
  /**
   * Upload an image to Gatsby project
   * @param {object} auth - Authentication credentials
  ...
  async uploadImage(auth, fileBuffer, filename) {
    try {
      ...
      console.error('Failed to upload image to Gatsby project:', error);
  ...
  async buildProject(auth) {
    try {
      ...
      return {
        success: true,
        message: 'Gatsby project built successfully',
        metadata: {
          stdout: stdout,
          stderr: stderr
        }
      ...
      console.error('Failed to build Gatsby project:', error);
      return {
        success: false,
        error: error.message,
  ...
  _parseMarkdownFile(content, filename) {
    try {
      ...
      return {
        ...
        link: `https://yourdomain.com/blog/${frontMatter.slug || filename.replace(/\.(md|mdx)$/, '')}`,
...
module.exports = new GatsbyAdapter();