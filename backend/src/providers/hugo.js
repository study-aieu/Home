/**
 * Hugo Provider Adapter
 * 
 * Supports publishing to Hugo static site generator
...
class HugoAdapter {
  constructor() {
    ...
    this.supports = {
      images: true,
      tags: true,
      scheduling: false, // Hugo doesn't support scheduling
  ...
  /**
   * Verify connection to Hugo site
    ...
    try {
      ...
      try {
        ...
        if (!configExists) {
          return {
            ok: false,
            details: 'Hugo site not found at specified path'
          };
        }

        // Check if it's a valid Hugo site
        const configContent = await fs.readFile(configPath, 'utf8');
        if (!configContent.includes('baseURL') && !configContent.includes('title')) {
          return {
            ok: false,
            details: 'Invalid Hugo configuration file'
          };
        }

        // Check if posts directory exists
        const postsPath = path.join(sitePath, 'content', 'posts');
        const postsExists = await fs.access(postsPath).then(() => true).catch(() => false);

        // Check if images directory exists
        const imagesPath = path.join(sitePath, 'static', 'images');
        ...
        return {
          ok: false,
          details: `Failed to verify Hugo site: ${error.message}`
        };
      }
    } catch (error) {
      return {
  ...
  async listPosts(auth, opts = {}) {
    try {
      ...
      console.error('Failed to list Hugo posts:', error);
      throw new Error(`Failed to fetch posts: ${error.message}`);
    }
  }

  /**
   * Create a new post on Hugo site
   * @param {object} auth - Authentication credentials
  ...
  async createPost(auth, input) {
    try {
      ...
      return {
        ...
        message: 'Post created successfully on Hugo site',
        metadata: {
          filename: filename,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to create Hugo post:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Update an existing post on Hugo site
   * @param {object} auth - Authentication credentials
   * @param {string} postId - Hugo post filename
  ...
  async updatePost(auth, postId, input) {
    try {
      ...
      return {
        ...
        postId: postId,
        externalId: postId,
        url: `https://yourdomain.com/posts/${postId.replace('.md', '')}`,
        message: 'Post updated successfully on Hugo site',
        metadata: {
          filename: postId,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to update Hugo post:', error);
      return {
        success: false,
        error: error.message,
  ...
  async deletePost(auth, postId) {
    try {
      ...
      return {
        ok: true,
        message: 'Post deleted successfully from Hugo site',
        metadata: {
          gitCommit: gitCommit
        }
      };
    } catch (error) {
      console.error('Failed to delete Hugo post:', error);
      return {
        ok: false,
  ...
  async uploadImage(auth, fileBuffer, filename) {
    try {
      ...
      console.error('Failed to upload image to Hugo site:', error);
  ...
  _parseMarkdownFile(content, filename) {
    try {
      ...
      return {
        ...
        link: `https://yourdomain.com/posts/${frontMatter.slug || filename.replace('.md', '')}`,
...
module.exports = new HugoAdapter();