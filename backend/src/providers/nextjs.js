/**
 * Next.js Provider Adapter
 * 
 * Supports publishing to Next.js via Git integration
...
class NextJSAdapter {
  constructor() {
    this.id = 'nextjs';
    this.name = 'Next.js';
    this.category = 'framework';
    
    // Define supported features
    this.supports = {
      images: true,
      tags: true,
      scheduling: false, // Next.js doesn't support scheduling
  ...
  /**
   * Verify connection to Next.js project
    ...
    try {
      ...
      try {
        ...
        if (!packageExists) {
          return {
            ok: false,
            details: 'Next.js project not found at specified path'
          };
        }
        ...
        if (!packageJson.dependencies?.next && !packageJson.devDependencies?.next) {
          return {
            ok: false,
            details: 'Invalid Next.js project - missing next dependency'
          };
        }
        ...
        return {
          ok: false,
          details: `Failed to verify Next.js project: ${error.message}`
        };
      }
    } catch (error) {
      return {
  ...
  /**
   * List posts from Next.js project
   * @param {object} auth - Authentication credentials
  ...
  async listPosts(auth, opts = {}) {
    try {
      ...
      console.error('Failed to list Next.js posts:', error);
      throw new Error(`Failed to fetch posts: ${error.message}`);
    }
  }

  /**
   * Create a new post on Next.js project
   * @param {object} auth - Authentication credentials
  ...
  async createPost(auth, input) {
    try {
      ...
      return {
        ...
        message: 'Post created successfully on Next.js project',
        metadata: {
          filename: filename,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to create Next.js post:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Update an existing post on Next.js project
   * @param {object} auth - Authentication credentials
   * @param {string} postId - Next.js post filename
  ...
  async updatePost(auth, postId, input) {
    try {
      ...
      return {
        ...
        message: 'Post updated successfully on Next.js project',
        metadata: {
          filename: postId,
          filePath: filePath,
          gitCommit: gitCommit
      ...
      console.error('Failed to update Next.js post:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Delete a post from Next.js project
   * @param {object} auth - Authentication credentials
   * @param {string} postId - Next.js post filename
  ...
  async deletePost(auth, postId) {
    try {
      ...
      return {
        ok: true,
        message: 'Post deleted successfully from Next.js project',
        metadata: {
          gitCommit: gitCommit
        }
      };
    } catch (error) {
      console.error('Failed to delete Next.js post:', error);
      return {
        ok: false,
  ...
  /**
   * Upload an image to Next.js project
   * @param {object} auth - Authentication credentials
  ...
  async uploadImage(auth, fileBuffer, filename) {
    try {
      ...
      console.error('Failed to upload image to Next.js project:', error);
  ...
  async buildProject(auth) {
    try {
      ...
      return {
        success: true,
        message: 'Next.js project built successfully',
        metadata: {
          stdout: stdout,
          stderr: stderr
        }
...
module.exports = new NextJSAdapter();